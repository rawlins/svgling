...
class DoubleBrackets(object):
    ...
    def width(self):
        return self.content.width() + (self.bracket_width + self.padding) * 2

    def height(self):
        return self.content.height() + 2
    ...
    def get_svg(self, name="figure"):
        ...
        container.fit()
        fig_box = svgwrite.container.SVG(x=(self.bracket_width + self.padding),
                                         y=1,
                                         width=self.content.width(),
                                         height=self.content.height())
        fig_box.add(self.content.get_svg())
        container.add(fig_box)
        svg_double_bracket(container, 0, 0, self.bracket_width, height)
        svg_double_bracket(container, width, 0, -self.bracket_width, height)
        return container
